...
import { extractHashtags, formatRelative, truncateHex } from "@/lib/utils";
...
export function NoteCard({ event, metricValue, metricLabel = "trend", highlight }: NoteCardProps) {
  ...
  return (
    <div className={`flex h-full flex-col rounded-3xl border border-white/10 ${highlight ? "bg-gradient-to-br from-white/10 via-white/5 to-transparent" : "bg-card/70"} p-5 shadow-2xl backdrop-blur`}> 
      <div className="flex items-center justify-between text-xs uppercase tracking-[0.2em] text-white/60">
        <span>{truncateHex(event.pubkey)}</span>
